...
if (!botToken || !ADMIN_USERNAME || !ADMIN_TELEGRAM_ID_FOR_NOTIFY) {
  console.error(
    'КРИТИЧЕСКАЯ ОШИБКА: Одна из переменных (BOT_TOKEN, ADMIN_USERNAME, ADMIN_TELEGRAM_ID_FOR_NOTIFY) не найдена в .env!'
  );
  process.exit(1);
}
...
bot.start(async (ctx) => {
  try {
    ...
    await ctx.reply(`Привет, ${userInDb.first_name}! 🚀\n\nВыберите действие:`, {
      reply_markup: {
        inline_keyboard: [
...
bot.command('admin', async (ctx) => {
  ...
  try {
    if (!isAdmin(ctx.from.username)) {
      await ctx.reply(`❌ Доступ отклонен. Ваш username: @${ctx.from.username}`);
      return;
    }
